...
class Dashboard {
    ...
    public function fetch_BarStackedChart_data() {
		...
		$get_list_of_imported_types = $wpdb->get_col($wpdb->prepare("select distinct( import_type ) from smackuci_events", array()));
		// $get_list_of_imported_types = $wpdb->get_col("select distinct( import_type ) from smackuci_events");
        
        $count = 1;
		foreach($get_list_of_imported_types as $import_type) {
            
			$get_chart_data = $wpdb->get_results( $wpdb->prepare( "select sum(created) as created, sum(updated) as updated, sum(skipped) as skipped from smackuci_events where import_type = %s", $import_type) );
