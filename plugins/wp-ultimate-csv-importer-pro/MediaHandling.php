...
class MediaHandling{
	...
	public function media_handling($img_url , $post_id , $data_array = null  ,$module = null, $image_type = null ,$hash_key = null, $header_array = null , $value_array = null, $wpml_array = null, $image_metas = null){
		...
		if($media_handle['media_settings']['media_handle_option'] == 'true'){
					...
					foreach($attachment_id as $value){
						if(!empty($wpml_array['language_code'])){
							...
							if(!wp_get_attachment_url($attach_id)){
								$attach_id = $this->image_function($img_url , $post_id , $data_array, '', 'use_existing_image',$header_array , $value_array, $wpml_array, $hash_key, $image_metas);	
							}else{
								if(!empty($data_array['featured_image'])) {
									set_post_thumbnail( $post_id, $attach_id );
								}
			...
			elseif($media_handle['media_settings']['overwriteImage'] == 'true'){
					...
					$attach_id = $this->image_function($img_url , $post_id , $data_array,'','',$header_array , $value_array, [], $hash_key, $image_metas);
				}
			}
			else{
				$attach_id = $this->image_function($img_url , $post_id , $data_array,'','',$header_array , $value_array, [], $hash_key, $image_metas);
	...
	public function image_function($f_img , $post_id , $data_array = null,$option_name = null, $use_existing_image = false,$header_array = null , $value_array = null, $wpml_array = null, $hash_key = null, $image_metas = null){
		global $wpdb;
		...
		if(!empty($header_array) && !empty($value_array) ){
			...
			if(!empty($hash_key)){
				$get_array_value = get_option('smack_media_seo'.$hash_key);
		...
		if(isset($media_settings[$media_handle['media_settings']['alttext']])) {
		...
		if(!empty($media_handle['media_settings']['file_name']) && !empty($media_settings[$media_handle['media_settings']['file_name']])){	
		...
		if($use_existing_image){
			...
			$attachment_ids = $wpdb->get_results("SELECT ID FROM {$wpdb->prefix}posts WHERE post_type = 'attachment' AND guid LIKE '%$fimg_name%'", ARRAY_A);
	...
	public function image_meta_table_entry($post_values, $post_id ,$acf_wpname_element, $acf_csv_name, $hash_key, $plugin){
		...
		if($media_handle['media_settings']['use_ExistingImage'] == 'true'){
			...
			$check_acf_image = $wpdb->get_results("SELECT hash_key FROM {$wpdb->prefix}ultimate_csv_importer_media_report WHERE hash_key = '$hash_key'  AND image_type = '{$image_type}' "); 
			if(empty($check_acf_image)){
			...
			$attachment_id = $wpdb->get_var("SELECT ID FROM ".$wpdb->prefix."posts WHERE post_type = 'attachment' AND guid LIKE '%$fimg_name'");
			if($attachment_id){
				$attach_id = $this->media_handling( $acf_csv_name, $post_id ,$post_values,'','',$hash_key);	
				...
				$acf_csv_name = WP_PLUGIN_URL . '/wp-ultimate-csv-importer-pro/assets/images/loading-image.jpg';	
				// $attach_id = $this->media_handling($acf_csv_name, $post_id, $acf_wpname_element);
				//added	
				$attach_id = $this->media_handling($acf_csv_name, $post_id, $post_values);	
			}
		}
		else{
			$check_acf_image = $wpdb->get_results("SELECT hash_key FROM {$wpdb->prefix}ultimate_csv_importer_media_report WHERE hash_key = '$hash_key'  AND image_type = 'acf_image' "); 
			if(empty($check_acf_image)){
