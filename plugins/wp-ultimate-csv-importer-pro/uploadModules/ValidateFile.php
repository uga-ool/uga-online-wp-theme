...
class ValidateFile {
    ...
    public function validate_file_format($filename){
        $supported_file = array('csv' , 'xml' , 'zip' , 'txt', 'xlsx','xls','json');
           ...
           if($file_extension == 'xlsx'){                
                    ...
                    if($uploaded_filesize > $get_php_size){
                        ...
                        elseif($file_extension == 'json'){
                            $delimiter = $this->getFileDelimiter($file_path, 5);
                            $validate_csv = $this->validateJSON($file_path , $delimiter);
                                $message = "yes"; 
                        }else{
                            $message = "yes";
                        }
    ...
    public function validateJSON($file_path='', $delimiter=','){
        if(!file_exists($file_path) || !is_readable($file_path))
            return FALSE;
            $header = array();
            $data = array();
            ...
            if (($handle = fopen($file_path, 'r')) !== FALSE)
            {
                $getcontents = file_get_contents( $file_path );
                $json = is_string($getcontents) && is_array(json_decode($getcontents, true)) && (json_last_error() == JSON_ERROR_NONE) ? true : false;
