...
class WPToolsetImporter extends ToolsetImporter
{
	...
	function import($postId,$post_val,$hash_key) {
		...
		if ($fieldsAndGroups == 0) {
			$this->insertCustomFields($postId,$this->postType,$post_val, $hash_key);

		}else{
			$this->loopAndInsertField($postId,$fieldsAndGroups,0,true,$post_val,$hash_key);
		}
		return;                                

	}

	function loopAndInsertField($postId,$fieldsAndGroups,$index,$isParent,$post_val,$hash_key) {
				...
				if (isset($data[1]) && isset($data[2]) && $data[1]=='repeatable' && $data[2]=='group') {
					...
					if ($index < count($csvGroupsArray)) {
						for ($i=0; $i < count($csvGroupsArray) ; $i++) { 
							...
							$this->loopAndInsertField($childPostId, array($elementArray), $i,false,$post_val,$hash_key);
					...
					if (!$isParent) {
						if ($index < count($fieldsArray)) {
							...
							$this->checkFieldType($postId,$fieldValue,$fieldType,$metaKey,$post_val,$hash_key);
						}
					} else {
						$this->checkFieldType($postId,$fieldValues,$fieldType,$metaKey,$post_val,$hash_key);
					}
				}
			}
		}
	...
	function insertCustomFields($postId,$postType,$post_val, $hash_key){
		foreach ($this->dataArray as $key => $value) {
			if(isset($this->fieldType[$key]) && isset($this->metaData[$key])){
			    ...
			    if(!empty($fieldType) && !empty($metaKey)) {
					...
					$this->checkFieldType( $postId, $value, $fieldType, $metaKey,$post_val,$hash_key );
			    }
			   }
			}
	...
	function checkFieldType($postId,$fieldValue,$fields_Type,$metaKey,$post_val,$hash_key){
		global $wpdb;
		if ($fields_Type == 'checkboxes') {
			...
			$this->InsertUpdateData($postId,$metaKey,$fields_Type,$fieldValue,$post_val, $hash_key);
		}
	...
	function InsertUpdateData($postId,$metaKey,$fields_Types,$fieldValue,$post_val,$hash_key)
	{  
		global $wpdb;
		settype($isRepetitive, "integer");
		$fieldTypeArray=array_flip($this->metaData);
		...
		if($fields_Types == 'image' || $fields_Types == 'file') {
			...
			foreach($valuesArray as $value) {
				...
				if ($ext) {
					...
					$attachid = self::$mediaInstance->image_meta_table_entry($post_val, $postId, $metaKey, $value, $hash_key, 'types');
					$attachids[] =$attachid;
					if(empty($attachid)){
						if ($this->postType == 'Users') {
