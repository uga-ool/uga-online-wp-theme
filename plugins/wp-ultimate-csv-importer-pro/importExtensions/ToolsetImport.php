...
class ToolsetImport {
    ...
    function set_toolset_values($header_array ,$value_array , $map, $maps, $post_id , $type , $mode, $hash_key){
		$post_values = [];
		$helpers_instance = ImportHelpers::getInstance();
		$post_values = $helpers_instance->get_header_values($map , $header_array , $value_array);
		$post_val = $helpers_instance->get_meta_values($maps , $header_array , $value_array);
		$this->types_import_function($post_values,$post_val,$type, $post_id, $mode, $hash_key);

	}

	public function types_import_function($data_array,$post_val, $type, $postId ,$mode, $hash_key) 
	{
		...
		{  
			...
			if ($mode=='Update') {
                ...
                if(!empty($data_array)) {
					...
					$WPToolsetUpdater->update($postId,$post_val,$hash_key);
                }
			}elseif ($mode =='Insert') {
                ...
                if(!empty($data_array)) { 
					...
					$wpToolsetImporter->import($postId,$post_val,$hash_key);
                } 
			}
