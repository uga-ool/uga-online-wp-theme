...
class WooCommerceMetaImport {
	...
	public function woocommerce_meta_import_function ($data_array, $image_meta, $pID, $variation_id ,$import_type , $line_number , $mode, $header_array , $value_array , $core_array, $hash_key) {
		global $wpdb;
		...
		foreach ($data_array as $ekey => $eval) {
			...
			case 'product_image_gallery' :
				...
				if(!empty($data_array[$ekey])){
					...
					foreach($get_all_gallery_images as $gallery_image) {
						...
						//    WooCommerceMetaImport::$media_instance->acfimageMetaImports($attachmentId,$image_meta,$plugin);
						// 	$gallery_image_ids .= $attachmentId . ',';


							$attachmentId = WooCommerceMetaImport::$media_instance->image_meta_table_entry($image_meta, $pID, 'product_image_gallery', $gallery_image, $hash_key, 'product');	
			...
			case 'custom_attributes' :
				if ($data_array[$ekey]) {
					...
					foreach($cusattribute as $cusattrkey){
						...
						update_post_meta($pID, $custom_attributes_name, $cus_attri_value);
                        $post_ID = $pID+1;
						global $wpdb;
			...
			case 'order_shipping' :
				...
				//$taxData['shipping_tax_amount'] = $data_array[$ekey];
				break;
			case 'cart_discount' :
		...
		if(isset($Shipment_name) && is_array($Shipment_name)){
			...
			if($mode == 'Update'){
				...
				if(count($order_ship->get_items('shipping')) > count($value_shipment)){
		...
		if (!empty($attribute_values)) {
			...
			for($i=0 ; $i< count($exploded_att_values) ; $i++) {	
				...
				foreach ($term_val as $term_value) {
					...
					//$slug_val[0]->slug = isset($slug_val[0]->slug) ? $slug_val[0]->slug : '';
					//$slug_value = $slug_val[0]->slug;
