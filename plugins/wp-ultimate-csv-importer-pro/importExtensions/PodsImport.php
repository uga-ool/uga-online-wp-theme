...
class PodsImport {
	...
	public function set_pods_values($header_array ,$value_array , $map, $maps, $post_id , $type, $hash_key, $lang_map = null){	
		$post_values = [];
		...
		$this->pods_import_function($post_values, $postimg, $type, $post_id , $header_array , $value_array, $lang_values, $hash_key);
	}

	public function pods_import_function($data_array, $postimg, $importas, $pID, $header_array , $value_array, $wpml_array, $hash_key) {
		global $wpdb;
		$helpers_instance = ImportHelpers::getInstance();
			...
			if($podsFields["PODS"][$custom_key]['type'] == 'file' || $podsFields["PODS"][$custom_key]['type'] == 'avatar'){
				...
				$ext = pathinfo($file, PATHINFO_EXTENSION);
					if($ext){
						...
						$get_file_id = $media_instance->image_meta_table_entry($postimg, $pID, $custom_key, $file, $hash_key, 'pods');
						if($get_file_id != '') {
							$gallery_ids[] = $get_file_id;
						}
