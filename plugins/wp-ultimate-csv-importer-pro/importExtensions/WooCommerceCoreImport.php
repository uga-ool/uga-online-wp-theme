...
class WooCommerceCoreImport {
    ...
    public function woocommerce_variations_import($data_array , $mode , $check , $hash_key , $line_number, $variation_count) {
		global $wpdb;
		...
		switch ($variation_condition) {
			case 'update_using_variation_id_and_sku':
				...
				if ( ! empty( $get_variation_data ) && $get_variation_data[0]->post_id == $variation_id ) {
					$returnArr = $this->importVariationData( $product_id, $variation_id, 'update_using_variation_id_and_sku' ,$hash_key  , $line_number, $variation_count,$get_variation_data);
				} else {
					$returnArr = $this->importVariationData( $product_id, $variation_id, 'default' ,$hash_key , $line_number, $variation_count, $productInfo);
				}
				break;
			case 'update_using_variation_id':
				...
				if ( ! empty( $get_variation_data ) && $get_variation_data[0]->ID == $variation_id ) {
					$returnArr = $this->importVariationData( $product_id, $variation_id, 'update_using_variation_id' ,$hash_key  , $line_number, $variation_count, $get_variation_data);
				} else {
					$returnArr = $this->importVariationData( $product_id, $variation_id, 'default',$hash_key , $line_number, $variation_count, $productInfo );
				}
				break;
			case 'update_using_variation_sku':
				...
				$get_variation_data = $wpdb->get_results( $wpdb->prepare( "select * from {$wpdb->prefix}posts where ID = %d and post_type = %s", $variation_id, 'product_variation' ) );
				if ( ! empty( $get_variation_data ) && $get_variation_data[0]->ID == $variation_id) {
					$returnArr = $this->importVariationData( $product_id,$variation_id, 'update_using_variation_sku' ,$hash_key  , $line_number, $variation_count,$get_variation_data);
				} else {
					$returnArr = $this->importVariationData( $product_id, $variation_id, 'default' ,$hash_key , $line_number, $variation_count, $productInfo);
				}
				break;
			case 'insert_using_product_id':
				$returnArr = $this->importVariationData( $product_id, $variation_id, 'insert_using_product_id',$hash_key , $line_number, $variation_count,  $productInfo);
				break;
			case 'insert_using_product_sku':
				$returnArr = $this->importVariationData( $product_id, $variation_id, 'insert_using_product_sku',$hash_key , $line_number, $variation_count, $productInfo );
				break;
			default:
				$returnArr = $this->importVariationData( $product_id, $variation_id, 'default',$hash_key  , $line_number, $variation_count, $productInfo);
				break;
	...
	public function importVariationData ($product_id, $variation_id, $type,$hash_key , $line_number, $variation_count, $exist_variation_data = array()) {
		global $wpdb;
		...
		if($type == 'default' || $type == 'insert_using_product_id' || $type == 'insert_using_product_sku') {
			...
			if(empty($variation_count)){
				...
				$total_variation = $wpdb->get_results( "SELECT id FROM {$wpdb->prefix}posts WHERE post_parent = '$parent_id' " , ARRAY_A);
				
				$total_variations = array_column($total_variation, 'id');
				$product_variation = implode(',', $total_variations);
				
				$core_instance->detailed_log[$line_number]['Message'] = 'Inserted Product ID: ' . $parent_id . '   Inserted Variation ID: ' . $product_variation;
			}
	...
	public function woocommerce_product_import($data_array, $mode, $type, $unmatched_row , $check , $hash_key , $line_number  , $acf,$pods, $toolset, $header_array ,$value_array, $wpml_values = null) {
