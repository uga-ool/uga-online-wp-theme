...
class TaxonomiesImport {
    ...
    public function taxonomies_import_function ($data_array, $mode, $importType , $unmatched_row, $check , $hash_key , $line_number ,$header_array ,$value_array) {
		$returnArr = array();
			...
			if($unmatched_row == 'true'){
				...
				if(!empty($termID)){
					...
					if(isset($parent_term_id)){
						...
						$wpdb->get_results("UPDATE $log_table_name SET created = $skipped_count WHERE hash_key = '$hash_key'");
