...
class WooCommerceExport {
				...
				foreach ($result2 as $key2 => $value2) {
					if($value->order_item_type != 'fee' && $value->order_item_type != 'shipping' && $value->order_item_type != ' fee' ){
						...
						if($value2->meta_key == '_line_tax_data'){
							$line_tax = '';
							if(is_serialized($value2->meta_value)){
								$unser =unserialize($value2->meta_value);
								foreach($unser as $unserkey => $unserval){
									foreach($unserval as $unserkey1 => $unserval1){
										$item = $unserkey1;
									}
									
								}
								
								$line_tax .= $unser['total'][$item].','.$unser['subtotal'][$item];
							}
							$line_tax_data .= $line_tax.'|';
	...
	public function getProductData($id, $type, $optionalType)
	{
		...
		if($type == 'WooCommerce'){	
			...
			$result = $wpdb->get_results($query);
			$var_query = $wpdb->prepare("SELECT meta_value FROM {$wpdb->prefix}postmeta where post_id = %d and meta_key = '_stock'", $id);
			$var_result = $wpdb->get_results($var_query);
			if($var_result[0]){
				WooCommerceExport::$export_instance->data[$id]['stock_qty'] = $var_result[0]->meta_value;
			}
			$term_relationship_arr = [];
			...
			foreach ($result as $key => $value) {	
				if($value->meta_key == '_product_attributes'){
					...
					foreach ($attArray as $key1 => $value1) {
							...
							if(empty($result)) {	
								$attvalues=$value1['value'];
								$attvalue .= $attvalues  . ',';	
								$attnames=$value1['name'];
								$attname .= $attnames.'|';
				...
				if($value->meta_key == '_stock' && $type !='WooCommerceVariations' ){
					WooCommerceExport::$export_instance->data[$id]['stock_qty'] = $value->meta_value; 
				}
				if($value->meta_key == '_chained_product_detail'){
					$chainedproduct = '';
					$unserialize = unserialize($value->meta_value);
					ksort($unserialize);
					foreach($unserialize as $unsermeta){
						$chainedproduct .= $unsermeta['product_name'].','.$unsermeta['unit'].'|';
						
					}
					$chained_details = substr($chainedproduct, 0 , -1);
					WooCommerceExport::$export_instance->data[$id]['_chained_product_detail'] = $chained_details; 
				
				}
				if($value->meta_key == '_chained_product_manage_stock'){
					WooCommerceExport::$export_instance->data[$id]['_chained_product_manage_stock'] = $value->meta_value; 
	...
	if(is_plugin_active('woocommerce-product-bundles/woocommerce-product-bundles.php')){
		...
		if(!empty($bundle_meta_result)){
			foreach($bundle_meta_result as $bundle_meta_value){
				...
				if(!empty($bundle_field_result)){
					foreach($bundle_field_result as $bundle_field_value){
						...
						if($bundle_field_value->meta_key == 'single_product_price_visibility'){
							$single_product_price_visibility = $bundle_field_value->meta_value;
							if($single_product_price_visibility == 'visible'){
								$product_price_value[] = 'Yes';
							}elseif($single_product_price_visibility == 'hidden'){
								$product_price_value[] = 'No';
							}
						}
						if($bundle_field_value->meta_key == 'cart_price_visibility'){
							$cart_price_visibility = $bundle_field_value->meta_value;
							if($cart_price_visibility == 'visible'){
								$cart_price[] = 'Yes';
							}elseif($cart_price_visibility == 'hidden'){
								$cart_price[] = 'No';
							}
						}
						if($bundle_field_value->meta_key == 'order_price_visibility'){
							$order_price_visibility = $bundle_field_value->meta_value;
							if($order_price_visibility == 'visible'){
								$order_price[] = 'Yes';
							}elseif($order_price_visibility == 'hidden'){
								$order_price[] = 'No';
							}
						}
			...
			$product_price_values = implode('|', $product_price_value);
			WooCommerceExport::$export_instance->data[$id]['single_product_price_visibility'] = $product_price_values;
			$cart_price_values = implode('|', $cart_price);
			WooCommerceExport::$export_instance->data[$id]['cart_price_visibility'] = $cart_price_values;
			$order_price_values = implode('|', $order_price);
			WooCommerceExport::$export_instance->data[$id]['order_price_visibility'] = $order_price_values;
		...
		foreach ($cus as $cus1) {
            if(!empty($cus1)){
                $custname = 'attribute_'.$cus1;
                $cust_name = $wpdb->get_var("SELECT attribute_label FROM {$wpdb->prefix}woocommerce_attribute_taxonomies WHERE attribute_name = '$cus1' ");
               
                if(!empty($cust_name)){
                    $custname = 'attribute_'.'pa_'.$cus1;
                }
                $cust_val = $wpdb->get_var("SELECT meta_value FROM {$wpdb->prefix}postmeta WHERE meta_key = '$custname' and post_id ='$id'");
                $cusAttr .= $cus1.'|'.$cust_val.','; 
            }
        }
		...
		WooCommerceExport::$export_instance->data[$id]['product_type'] = isset(WooCommerceExport::$export_instance->data[$id]['product_type']) ? WooCommerceExport::$export_instance->data[$id]['product_type'] : '';
        ...
        WooCommerceExport::$export_instance->data[$id]['_tax_status'] = isset(WooCommerceExport::$export_instance->data[$id]['_tax_status']) ? WooCommerceExport::$export_instance->data[$id]['_tax_status'] : '';
		...
		WooCommerceExport::$export_instance->data[$id]['_tax_class'] = isset( WooCommerceExport::$export_instance->data[$id]['_tax_class']) ?  WooCommerceExport::$export_instance->data[$id]['_tax_class'] : '';
		$tax_class = WooCommerceExport::$export_instance->data[$id]['_tax_class'];
		WooCommerceExport::$export_instance->data[$id]['_backorders'] = isset( WooCommerceExport::$export_instance->data[$id]['_backorders']) ?  WooCommerceExport::$export_instance->data[$id]['_backorders'] : '';
	 ...
	 * @return array
	 */
	public function FetchTaxonomies($module,$optionalType, $is_filter,$mode = null) {	
		global $wpdb;
		...
		$query=$wpdb->get_results("SELECT t.name, t.slug, tx.description, tx.parent, t.term_id FROM {$wpdb->prefix}terms as t join {$wpdb->prefix}term_taxonomy as tx on t.term_id = tx.term_id where tx.taxonomy='$taxonomy'");
		self::$export_instance->totalRowCount = count($query);
