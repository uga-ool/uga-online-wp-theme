...
class JetEngineCCTExtension extends ExtensionHandler{
	...
	*/
	public function JetEngineCCTFields($import_type) {	
		global $wpdb;
		$get_meta_fields = $wpdb->get_results($wpdb->prepare("SELECT id, meta_fields FROM {$wpdb->prefix}jet_post_types WHERE slug = %s AND status = %s", $import_type, 'content-type'));
			...
			foreach($unserialized_meta as $jet_key => $jet_value){
				...
				if($jet_field_type == 'repeater'){
					...
					foreach($jet_rep_fields as $jet_rep_fkey => $jet_rep_fvalue){
						...
						$customFields["JECCTRF"][ $jet_rep_fkey ]['label'] =$jet_field_label;
						$customFields["JECCTRF"][ $jet_rep_fkey ]['name']  = $jet_field_name;
					}
				}
			}
		}
